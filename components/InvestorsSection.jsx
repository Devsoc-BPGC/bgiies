...
export default function InvestorsSection() {
  ...
  // --- SCROLL & DRAG LOGIC (From OurPartners) ---
  const isDraggingRef = useRef(false);
  const startXRef = useRef(0);
  const startScrollRef = useRef(0);
  const rafRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (!isDraggingRef.current) {
        ...
        // Reset scroll once we pass the halfway point (end of first set)
        if (el.scrollLeft >= el.scrollWidth / 2) {
          el.scrollLeft = 0;
        }
      }
  ...
  return (
    ...
    >
      ...
      {/* Scroll Container - Logic applied here */}
      <div
        ref={scrollRef}
        className="overflow-hidden w-full cursor-grab select-none"
        style={{ whiteSpace: "nowrap" }}
      >
        <div className="flex gap-10 py-6 w-fit pl-4">
          {loopItems.map((img, idx) => (
            <div
              key={idx}
              className="w-56 h-40 flex-shrink-0 flex items-center justify-center bg-white rounded-xl shadow-md border border-gray-100 whitespace-normal"
            >
              <img 
                src={img} 
